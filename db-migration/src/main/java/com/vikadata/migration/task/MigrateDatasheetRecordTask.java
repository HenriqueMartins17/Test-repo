...
@Slf4j
@Component
public class MigrateDatasheetRecordTask implements Runnable {

    @Resource
    private DataMigrationCursorService migrationCursorService;

    @Resource
    private DatasheetRecordService service;
    ...
    public void run() {
        ...
        DataMigrationCursor migrationCursor = migrationCursorService
            .getMigrationCursor(DatasheetRecordService.COLLECTION_NAME);

        String lastMigrateId = null == migrationCursor ? null : migrationCursor.getCursorId();

        log.info("start to migrate datasheet_record from {} ", lastMigrateId);
        ...
        while (true) {
            ...
            try {
                ...
                if (lastMigrateId.equals(start)) {
                    log.info("migrate datasheet_record finished");
                    migrationCursorService.updateCursorId(DatasheetRecordService.COLLECTION_NAME
            ...
            if (!lastMigrateId.equals(start)) {
                log.info("migrate datasheet_record from {} to {}", start, lastMigrateId);
            }
        }
    }
