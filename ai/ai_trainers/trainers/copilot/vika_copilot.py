...
class VikaCopilotTrainer(BaseRAGTrainer):

    """
    Train Vikadata Copilot via help center documents
    Output: ./data/vika_copilot
    """

    def __init__(self, datasheet_id: str, help_center_host: str, repo_name: str, verbose=False) -> None:
        super().__init__("vika_copilot")
    ...
    def split_markdown_by_header(self, docs: list[Document], path: str) -> list[Document]:
        ...
        for doc in docs:
            page_url = self._extract_url_from_slug(doc.page_content, path)
            ...
            splits_in_speficied_length = []
            for split in splits:
                #print(f"split: {split}\n")
                texts = text_splitter.create_documents([split.page_content])

                #for i, text in enumerate(texts):
                    #print(f"\ttext#{i}: {text}\n")

                splits_in_speficied_length.extend(texts)
            
            #print(f"splits_in_speficied_length: {splits_in_speficied_length}")

            source = page_url if page_url else doc.metadata["source"]
            for index, split in enumerate(splits_in_speficied_length):
    ...
    def _extract_all_images_from_page(self, page_url: str) -> dict:
        ...
        self.verbose and print(f"Fetching page url: {page_url}")
        ...
        try:
            for i in range(0, len(documents), 5):
                ...
                if response_json["success"] is True:
                    ...
                    # sleep 300ms second to avoid rate limit
                    time.sleep(0.3)
                else:
