...
public class TemplateCenterConfigServiceImpl implements ITemplateCenterConfigService {
    ...
    public void updateTemplateCenterConfig(Long userId, TemplateCenterConfigRo ro) {
        ...
        List<TemplateCenterConfigInfo> configInfos =
            vikaOperations.getTemplateCenterConfigInfos(ro.getHost(), ro.getToken(),
                BeanUtil.copyProperties(ro, TemplateConfigDatasheetParam.class));
        ...
        configInfos.forEach(info -> {
            Optional<Template> template = info.getTemplate().stream()
                .filter(item -> !tplNameToTplIdMap.containsKey(item.getName())).findFirst();
            template.ifPresent(tpl -> {
                throw new BusinessException(
                    StrUtil.format("Template「{}」is not exist!", tpl.getName()));
            });
        });
        ...
        existingData.i18nToRecommendMap.keySet().forEach(lang -> {
            templateConfigCacheService.deleteRecommendConfigCacheByLang(lang);
            templateConfigCacheService.deleteCategoriesListConfigCacheByLang(lang);
        });
    }
    ...
    private void computeDisuseChangeSet(TemplateExistingData existingData,
                                        TemplateChangeSet changeSet) {
        // recommend
        List<Long> retainedUsedConfigIds =
            changeSet.updateSystemConfigEntities.stream().map(SystemConfigEntity::getId)
        ...
        // album
        List<String> retainedUsedAlbumIds =
            changeSet.updateTemplateAlbumEntities.stream().map(TemplateAlbumEntity::getAlbumId)
                .collect(Collectors.toList());
    ...
    private void handleRecommend(Long userId, TemplateCenterConfigInfo configInfo,
                                 TemplateExistingData existingData, TemplateChangeSet changeSet) {
        ...
        if (CollUtil.isNotEmpty(recommend.getTop())) {
            List<Banner> top = recommend.getTop().stream()
                .map(item -> new Banner(tplNameToTplIdMap.get(item.getTemplateName()),
                    item.getImage(), item.getTitle(), item.getDesc(), item.getColor()))
                .collect(Collectors.toList());
        ...
        if (CollUtil.isNotEmpty(recommend.getAlbumGroups())) {
            ...
            Map<String, TemplateAlbumDto> albumNameToAlbumMap =
                existingData.getAlbumNameToAlbumMap(i18n);
            for (RecommendInfo.AlbumGroup group : recommend.getAlbumGroups()) {
                ...
                List<String> albumIds = group.getAlbumNames().stream()
                    .map(item -> albumNameToAlbumMap.get(item).getAlbumId())
                    .collect(Collectors.toList());
        ...
        if (CollUtil.isNotEmpty(recommend.getTemplateGroups())) {
            ...
            for (RecommendInfo.TemplateGroup group : recommend.getTemplateGroups()) {
                ...
                List<String> templateIds = group.getTemplateNames().stream()
                    .map(item -> existingData.tplNameToTplIdMap.get(item))
                    .collect(Collectors.toList());
    ...
    private void handleTemplateCategory(Long userId, TemplateCenterConfigInfo configInfo,
                                        TemplateExistingData existingData,
                                        TemplateChangeSet changeSet) {
        if (CollUtil.isEmpty(configInfo.getTemplateCategories())) {
            return;
        }
        String i18n = configInfo.getI18n();
        Map<String, TemplatePropertyDto> categoryNameToPropertyMap =
            existingData.getCategoryNameToPropertyMap(i18n);

        int order = 0;
        for (TemplateCategory category : configInfo.getTemplateCategories()) {
            // template category property
            TemplatePropertyDto propertyDto =
                this.handleTemplateProperty(userId, i18n, category.getName(),
                    TemplatePropertyType.CATEGORY, categoryNameToPropertyMap, changeSet);
            ...
            if (CollUtil.isNotEmpty(category.getAlbumNames())) {
                ...
                for (String albumName : category.getAlbumNames()) {
                    ...
                    this.createTemplateAlbumRel(albumId, TemplateAlbumRelType.TEMPLATE_CATEGORY,
                        propertyDto.getPropertyCode(), changeSet);
                }
            }
    ...
    private void handleTemplateTag(Long userId, TemplateCenterConfigInfo configInfo,
                                   TemplateExistingData existingData, TemplateChangeSet changeSet) {
        if (CollUtil.isEmpty(configInfo.getTemplate())) {
            return;
        }
        String i18n = configInfo.getI18n();
        Map<String, TemplatePropertyDto> tagNameToPropertyMap =
            existingData.getTagNameToPropertyMap(i18n);
        ...
        for (Template template : configInfo.getTemplate()) {
            ...
            for (String tagName : template.getTemplateTags()) {
                ...
                TemplatePropertyDto propertyDto =
                    this.handleTemplateProperty(userId, i18n, tagName, TemplatePropertyType.TAG,
                        tagNameToPropertyMap, changeSet);
                // create property rel
                this.createTemplatePropertyRel(templateId, propertyDto, order, changeSet);
                order++;
            }
    ...
    private TemplatePropertyDto handleTemplateProperty(Long userId, String i18n, String propName,
                                                       TemplatePropertyType type,
                                                       Map<String, TemplatePropertyDto> propNameToPropertyMap,
                                                       TemplateChangeSet changeSet) {
        ...
        String propCode = type == TemplatePropertyType.CATEGORY ? IdUtil.createTempCatCode() :
            IdUtil.createTempTagCode();
        ...
        TemplatePropertyDto propertyDto =
            new TemplatePropertyDto(propId, propName, propCode, type.getType(), i18n);
        propNameToPropertyMap.put(propName, propertyDto);
        return propertyDto;
    }

    private void createTemplatePropertyRel(String templateId, TemplatePropertyDto propertyDto,
                                           int order, TemplateChangeSet changeSet) {
    ...
    private void handleTemplateAlbum(Long userId, TemplateCenterConfigInfo configInfo,
                                     TemplateExistingData existingData,
                                     TemplateChangeSet changeSet) {
        ...
        for (TemplateAlbum album : configInfo.getAlbums()) {
            ...
            if (albumNameToAlbumMap.containsKey(album.getName())) {
                ...
                albumNameToAlbumMap.put(album.getName(),
                    new TemplateAlbumDto(albumEntity.getId(), albumId, i18n, album.getName()));
            }
            ...
            if (CollUtil.isNotEmpty(album.getTemplateNames())) {
                for (String templateName : album.getTemplateNames()) {
                    // create property rel
                    String templateId = existingData.tplNameToTplIdMap.get(templateName);
                    this.createTemplateAlbumRel(albumId, TemplateAlbumRelType.TEMPLATE, templateId,
                        changeSet);
                }
            }
            ...
            if (CollUtil.isNotEmpty(album.getTemplateTags())) {
                ...
                for (String tagName : album.getTemplateTags()) {
                    ...
                    this.createTemplateAlbumRel(albumId, TemplateAlbumRelType.TEMPLATE_TAG, tagCode,
                        changeSet);
                }
            }
        }
    ...
    private void createTemplateAlbumRel(String albumId, TemplateAlbumRelType type, String relateId,
                                        TemplateChangeSet changeSet) {
    ...
    private Map<String, String> getTplNameToTplIdMap(String spaceId) {
        ...
        return templateInfos.stream()
            .collect(Collectors.toMap(TemplateInfo::getName, TemplateInfo::getTemplateId));
    }
    ...
    class TemplateExistingData {
        ...
        public TemplateExistingData(Map<String, String> templateMap) {
            ...
            List<SystemConfigDTO> recommendConfigs = systemConfigMapper.selectConfigDtoByType(
                SystemConfigType.RECOMMEND_CONFIG.getType());
            ...
            allAlbumIds =
                albums.stream().map(TemplateAlbumDto::getAlbumId).collect(Collectors.toList());
            ...
            List<TemplatePropertyDto> templateProperties =
                templatePropertyMapper.selectAllTemplatePropertyDto();
            allPropertyCodes = templateProperties.stream().map(TemplatePropertyDto::getPropertyCode)
                .collect(Collectors.toList());
    ...
    class TemplateChangeSet {
        ...
        void doSaveOrUpdate(Long userId) {
            ...
            if (!updateTemplateAlbumEntities.isEmpty()) {
                updateTemplateAlbumEntities.forEach(
                    entity -> templateAlbumMapper.updateById(entity));
            }
            ...
            if (!retainedUsedPropertyCodes.isEmpty()) {
                templatePropertyMapper.updateUpdatedByByPropertyCodes(retainedUsedPropertyCodes,
                    userId);
            }
