...
public class IdaasContactChangeServiceImpl implements IIdaasContactChangeService {
    ...
    public void saveContactChange(String tenantName, String spaceId,
                                  IdaasContactChange contactChange) {
        ...
        if (CollUtil.isNotEmpty(addGroupResponses)) {
            ...
            CollUtil.split(addGroupResponses, 500).forEach(addGroups -> {
                ...
                List<IdaasGroupBindEntity> groupBindEntities =
                    Lists.newArrayListWithCapacity(addGroups.size());
                addGroups.forEach(addGroup -> {
                    Long teamId = IdWorker.getId();
                    Long rootTeamId = teamService.getRootTeamId(spaceId);
                    TeamEntity teamEntity =
                        OrganizationFactory.createTeam(spaceId, teamId, rootTeamId,
                            addGroup.getName(), addGroup.getOrder());
        ...
        if (CollUtil.isNotEmpty(updateGroupBinds)) {
            ...
            CollUtil.split(updateGroupBinds, 500).forEach(groupBinds -> {
                ...
                List<IdaasGroupBindEntity> groupBindEntities =
                    Lists.newArrayListWithCapacity(groupBinds.size());
        ...
        List<IdaasGroupBindEntity> effectiveGroupBinds =
            idaasGroupBindService.getAllBySpaceId(spaceId);
        Map<String, Long> groupBindTeamMap = effectiveGroupBinds.stream()
            .collect(
                Collectors.toMap(IdaasGroupBindEntity::getGroupId, IdaasGroupBindEntity::getTeamId,
                    (k1, k2) -> k2));
        Long rootTeamId = teamService.getRootTeamId(spaceId);
        // 5 Process new users
        ...
        if (CollUtil.isNotEmpty(addUserResponses)) {
            ...
            CollUtil.split(addUserResponses, 500).forEach(addUsers -> {
                ...
                List<TeamMemberRelEntity> teamMemberRelEntities =
                    Lists.newArrayListWithCapacity(addUsers.size());
                List<IdaasUserBindEntity> userBindEntities =
                    Lists.newArrayListWithCapacity(addUsers.size());
                addUsers.forEach(addUser -> {
                    ...
                    userBindEntities.add(IdaasUserBindEntity.builder()
                        ...
                        .email(addUserValues.getPrimaryMail())
                        .mobile(addUserValues.getPhoneNum())
                        .groupIds(CollUtil.isEmpty(addUserValues.getGroups()) ? null :
                            JSONUtil.toJsonStr(addUserValues.getGroups()))
                        .vikaUserId(userId)
        ...
        if (CollUtil.isNotEmpty(addMemberBinds)) {
            ...
            CollUtil.split(addMemberBinds, 500).forEach(addMembers -> {
                List<MemberEntity> memberEntities =
                    Lists.newArrayListWithCapacity(addMembers.size());
                List<TeamMemberRelEntity> teamMemberRelEntities =
                    Lists.newArrayListWithCapacity(addMembers.size());
                addMembers.forEach(addMember -> {
                    ...
                    memberEntities.add(MemberEntity.builder()
                        ...
                        .isAdmin(false)
                        .build());
                    List<TeamMemberRelEntity> teamMemberRels =
                        JSONUtil.toList(addMember.getGroupIds(), String.class).stream()
                            .filter(group -> Objects.nonNull(groupBindTeamMap.get(group)))
                            .map(group -> TeamMemberRelEntity.builder()
                                .memberId(memberId)
                                .teamId(groupBindTeamMap.get(group))
        ...
        if (CollUtil.isNotEmpty(updateUserBinds)) {
            ...
            CollUtil.split(updateUserBinds, 500).forEach(userBinds -> {
                ...
                List<MemberEntity> memberEntities =
                    Lists.newArrayListWithCapacity(userBinds.size());
                List<TeamMemberRelEntity> teamMemberRelEntities =
                    Lists.newArrayListWithCapacity(userBinds.size());
                List<IdaasUserBindEntity> userBindEntities =
                    Lists.newArrayListWithCapacity(userBinds.size());
                ...
                List<MemberEntity> members =
                    memberService.getByUserIdsAndSpaceId(vikaUserIds, spaceId);
                Map<Long, Long> userMemberIdMap = members.stream()
                    .collect(Collectors.toMap(MemberEntity::getUserId, MemberEntity::getId,
                        (k1, k2) -> k2));
                userBinds.forEach(userBind -> {
                    ...
                    memberEntities.add(MemberEntity.builder()
                        ...
                        .build());
                    List<TeamMemberRelEntity> teamMemberRels =
                        JSONUtil.toList(userBind.getGroupIds(), String.class).stream()
                            .filter(group -> Objects.nonNull(groupBindTeamMap.get(group)))
                            .map(group -> TeamMemberRelEntity.builder()
                                .memberId(memberId)
                                .teamId(groupBindTeamMap.get(group))
