...
import com.apitable.enterprise.social.filter.FeishuContextFilter;
...
public class FeishuAutoConfiguration
    implements ApplicationContextAware, BeanDefinitionRegistryPostProcessor, BeanPostProcessor,
    EnvironmentAware {
    ...
    @Bean
    FeishuContextFilter feishuContextFilter() {
        return new FeishuContextFilter();
    }
    ...
    @ConditionalOnProperty(value = "social.feishu.isv", havingValue = "true")
    ...
    @ConditionalOnProperty(prefix = "social.feishu", name = "app-id")
    ...
    public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry)
        ...
        BindResult<FeishuProperties> bind =
            Binder.get(environment).bind("social.feishu", Bindable.of(FeishuProperties.class));
        properties = bind.get();
        if (properties == null) {
            return;
        }
        ...
        if (bean instanceof AppTicketStorage && FEISHU_TICKET_STORAGE_BEAN_NAME.equals(beanName)) {
            ...
            if (properties.getIsv()) {
                ...
                instance.getEventListenerManager()
                    .registerEventCallbackHandler(AppTicketEvent.class,
                        new AppTicketEventCallbackHandler(ticketStorage));
            }
        }
        if (bean instanceof FeishuConfigStorage &&
            FEISHU_CONFIG_STORAGE_BEAN_NAME.equals(beanName)) {
    ...
    public void onFeishuEventListenerScan() {
        ...
        factory.getV3ContactEventHandlerMap()
            .forEach(manager::registerV3ContactEventCallbackHandler);
        ...
        if (LOGGER.isInfoEnabled()) {
            ...
            manager.getEventHandlerMap().keySet()
                .forEach(e -> output.append("Event:").append(e.getSimpleName()).append("\n"));
            manager.getV3ContactEventHandlerMap().keySet().forEach(
                e -> output.append("V3 Version Contact Event: ").append(e.getSimpleName())
                    .append("\n"));
            output.append("---------------------------------------------------");
        ...
        if (instance.getFeishuTemplate().getDefaultConfigStorage() != null) {
            FeishuConfigStorage configStorage =
                instance.getFeishuTemplate().getDefaultConfigStorage();
