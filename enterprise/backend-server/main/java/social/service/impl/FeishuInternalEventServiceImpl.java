...
public class FeishuInternalEventServiceImpl implements IFeishuInternalEventService {
    ...
    public void urlCheck(String appInstanceId) {
        log.info(
            "Post processing of event subscription address verification, application instance ID: {}",
            appInstanceId);
    ...
    public void syncContactFirst(FeishuPassportUserInfo userInfo, AppInstance appInstance) {
        ...
        boolean isTenantExist =
            iSocialTenantService.isTenantExist(userInfo.getTenantKey(), profile.getAppKey());
        if (!isTenantExist) {
            // Create Tenant Record
            iSocialTenantService.createTenant(SocialPlatformType.FEISHU, SocialAppType.INTERNAL,
                profile.getAppKey(), userInfo.getTenantKey(), new JSONObject().toString());
        }
        // Create Tenant Binding
        boolean bindExist =
            iSocialTenantBindService.checkExistBySpaceIdAndTenantId(profile.getAppKey(),
                appInstance.getSpaceId(), userInfo.getTenantKey());
        if (!bindExist) {
            iSocialTenantBindService.addTenantBind(profile.getAppKey(), userInfo.getTenantKey(),
        ...
        boolean isExist =
            iSocialUserBindService.isUnionIdBind(mainAdminUserId, userInfo.getUnionId());
        if (!isExist) {
        ...
        iSpaceInviteLinkService.deleteByTeamIds(Collections.singletonList(rootTeamId));
        List<Long> subTeamIds =
            iTeamService.getTeamIdsBySpaceId(appInstance.getSpaceId());
        subTeamIds.remove(rootTeamId);
        ...
        List<Long> readyRemovedMemberIds =
            iMemberService.getMemberIdsBySpaceId(appInstance.getSpaceId());
        readyRemovedMemberIds.removeIf(memberId -> memberId.equals(mainAdminMemberId));
        iMemberService.batchDeleteMemberFromSpace(appInstance.getSpaceId(), readyRemovedMemberIds,
            false);
        ...
        MultiValueMap<FeishuDeptObject, FeishuUserObject> contactMap =
            iFeishuTenantContactService.fetchTenantContact(userInfo.getTenantKey());
        if (contactMap.isEmpty()) {
            ...
            contactMap.add(FeishuContactAuthScope.createRootDeptObject(),
                userInfo.createUserObject());
        }
        handleTenantContactData(profile.getAppKey(), appInstance.getSpaceId(),
            userInfo.getTenantKey(), userInfo.getOpenId(), contactMap);
    ...
    public void handleContactScopeChangeEvent(ContactScopeUpdateEvent event) {
        ...
        iFeishuService.switchContextIfAbsent(
            iAppInstanceService.buildConfigStorageByInstanceId(event.getAppInstanceId()));
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void handleTenantContactData(String appId, String spaceId, String tenantKey,
                                        String installer,
                                        MultiValueMap<FeishuDeptObject, FeishuUserObject> contactMap) {
        ...
        Map<String, Long> deptIdGeneratorMap =
            generateDeptBindTeamIdIfAbsent(contactMap, bindTeamMap, spaceContext.getRootTeamId());
        ...
        // Members of tenants
        List<String> tenantUserOpenIds =
            iSocialTenantUserService.getOpenIdsByAppIdAndTenantId(appId, tenantKey);
        // Tenant's department
        List<SocialTenantDepartmentEntity> tenantDepartments =
            iSocialTenantDepartmentService.getByTenantId(tenantKey, spaceId);
        Map<String, Long> tenantDepartmentIdMap = tenantDepartments.stream()
            .collect(Collectors.toMap(SocialTenantDepartmentEntity::getDepartmentId,
                SocialTenantDepartmentEntity::getId));
        ...
        for (Entry<FeishuDeptObject, List<FeishuUserObject>> entry : contactMap.entrySet()) {
            ...
            if (!dept.getDepartmentId().equals(FEISHU_ROOT_DEPT_ID)) {
                ...
                Long parentTeamId = dept.getParentDepartmentId().equals(FEISHU_ROOT_DEPT_ID) ?
                    spaceContext.getRootTeamId() :
                    deptIdGeneratorMap.getOrDefault(dept.getParentDepartmentId(),
                        spaceContext.getRootTeamId());
                    ...
                    if (!tenantDepartmentIdMap.containsKey(dept.getDepartmentId())) {
                        ...
                        meta.teamEntities.add(
                            OrganizationFactory.createTeam(meta.spaceId, teamId, parentTeamId,
                                subDeptName(dept.getName()), Integer.parseInt(dept.getOrder())));
                ...
                if (StrUtil.isNotBlank(installer)) {
                    ...
                    if (installer.equals(user.getOpenId())) {
                        ...
                        meta.teamMemberRelEntities.add(
                            SocialFactory.createFeishuTeamMemberRel(teamId,
                                spaceContext.getMainAdminMemberId()));
                    } else {
        ...
        // Save Changes
        meta.doSaveOrUpdate(appId, tenantKey, installer);
    }
        ...
        try {
            if (lock.tryLock(100, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    log.error(
                        String.format("Tenant [%s] handing event [%s] fail, resolve it quickly",
                            tenantKey, event.getHeader().getEventType()), e);
                    throw e;
                } finally {
                    lock.unlock();
    ...
    private void handleUserActive(String appId, String tenantKey, String spaceId,
        ...
        handleUserDepartmentRelate(tenantKey, spaceId, member.getId(),
            userObject.getDepartmentIds());
    }
        ...
        try {
            if (lock.tryLock(100, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    log.error(
                        String.format("Tenant [%s] handling events [%s] fail, solve it quickly",
                            tenantKey, event.getHeader().getEventType()), e);
                    throw e;
                } finally {
                    lock.unlock();
    ...
    private void handleCreateDept(String tenantKey, String spaceId, FeishuDeptObject deptObject) {
        ...
        Long toCreateTeamId =
            createTeamIfNotExist(tenantKey, spaceId, parentDeptObject.getDepartmentId(),
                subDeptName(deptObject.getName()), deptObject.getOrder());
    ...
    public void handleDeptUpdateEvent(ContactDeptUpdateEvent event) {
        ...
        String spaceId = iSocialTenantBindService.getTenantDepartmentBindSpaceId(appId, tenantKey);
        if (StrUtil.isBlank(spaceId)) {
            log.error("[Department information update event] Tenant 「{}」not bind any space",
                tenantKey);
            return;
        }
        String lockKey = StrUtil.join("-", appId, tenantKey);
    ...
    private void handleUpdateDept(String spaceId, String tenantKey, FeishuDeptObject deptObject) {
        ...
        if (teamId == null) {
            ...
            if (bindParentTeamId == null) {
                log.warn(
                    "[Department information update event] Tenant「{}」department「{}」's superior department [{}] not bind",
                    tenantKey, deptObject.getDepartmentId(), deptObject.getParentDepartmentId());
                return;
            }
            // New Department
        ...
        try {
            if (lock.tryLock(100, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    log.error(
                        String.format("Tenant [%s] handling events [%s] fail, resole it quickly",
                            tenantKey, event.getHeader().getEventType()), e);
                    throw e;
                } finally {
                    lock.unlock();
    ...
    /**
     * Send a message card to the user when the number of seats is exceeded
     *
     * @param appId
     * @param tenantKey
     * @param userOpenId
