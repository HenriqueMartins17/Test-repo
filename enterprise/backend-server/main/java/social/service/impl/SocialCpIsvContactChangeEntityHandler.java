...
@Slf4j
@Service
public class SocialCpIsvContactChangeEntityHandler
    ...
    private void createUser(String tenantId, String appId, String cpUserId, String spaceId)
        throws WxErrorException {

        // 1 Obtain the visible range of current enterprise authorization
        SocialTenantEntity socialTenantEntity =
            socialTenantService.getByAppIdAndTenantId(appId, tenantId);
        Assert.notNull(socialTenantEntity, () -> new IllegalStateException(String
            .format("No available tenant information found,tenantId：%s，appId：%s", tenantId,
                appId)));
        ...
        if (viewable) {
            ...
            socialCpIsvService.sendWelcomeMessage(socialTenantEntity, spaceId, wxCpMessage,
                Collections.singletonList(cpUserId), null, null);
        }

    }

    private void updateUser(String tenantId, String appId, String cpUserId, String spaceId,
                            Integer[] cpDepartments)
        throws WxErrorException {
    ...
    private void updateTag(String tenantId, String appId, String spaceId,
                           String[] addUserItems, String[] delUserItems, Integer[] addPartyItems,
                           Integer[] delPartyItems)
        throws WxErrorException {
        ...
        if (ArrayUtil.isNotEmpty(addPartyItems)) {
            ...
            WxCpIsvUserServiceImpl wxCpTpUserService =
                (WxCpIsvUserServiceImpl) weComTemplate.isvService(appId)
                    .getWxCpTpUserService();
            // Use cache to prevent frequent calls to the interface during mass operation of the address book
            String userSimpleListKey = getWecomIsvContactUserSimpleListKey(tenantId);
            for (Integer party : addPartyItems) {
                ...
                if (CollUtil.isEmpty(cpUsers)) {
                    cpUsers = wxCpTpUserService
                        .listSimpleByDepartment(party.longValue(), true,
                            WeComUserStatus.ACTIVE.getCode(), tenantId);
