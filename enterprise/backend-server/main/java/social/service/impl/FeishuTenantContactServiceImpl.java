...
public class FeishuTenantContactServiceImpl implements IFeishuTenantContactService {
    ...
    @Override
    public MultiValueMap<FeishuDeptObject, FeishuUserObject> fetchTenantContact(String tenantKey)
        throws ContactAccessDeniesException {
        try {
            FeishuTenantContact contact = requestTenantContact(tenantKey);
            return fetchTenantContact(tenantKey, contact.getDeptObjects(),
                contact.getUserObjects());
        } catch (Exception exception) {
            log.error(
                "No Lark Enterprise Tenant[{}] any address book permission, gray scale test in progress",
                tenantKey, exception);
    ...
    public FeishuTenantContact requestTenantContact(String tenantKey) {
        ...
        if (CollUtil.isNotEmpty(authedOpenIds)) {
            ...
            authedOpenIds.forEach(
                openId -> userObjects.add(iFeishuService.getUser(tenantKey, openId)));
    ...
    @Override
    public MultiValueMap<FeishuDeptObject, FeishuUserObject> fetchTenantContact(String tenantKey,
                                                                                List<FeishuDeptObject> deptObjects,
                                                                                List<FeishuUserObject> userObjects) {
        ...
        if (CollUtil.isNotEmpty(deptObjects)) {
            for (FeishuDeptObject deptObject : deptObjects) {
                ...
                List<FeishuUserObject> users =
                    fetchUsersByDepartmentId(tenantKey, deptObject.getDepartmentId());
                ...
                FeishuV3DeptsPager subDeptsPager =
                    iFeishuService.getDeptPager(tenantKey, deptObject.getDepartmentId());
                while (subDeptsPager.hasNext()) {
                    List<FeishuDeptObject> subDepts = subDeptsPager.next();
                    for (FeishuDeptObject subDept : subDepts) {
                        List<FeishuUserObject> subDeptUsers =
                            fetchUsersByDepartmentId(tenantKey, subDept.getDepartmentId());
                        contactMap.addAll(subDept, subDeptUsers);
                        if (CollUtil.isNotEmpty(userObjects)) {
                            subDeptUsers.forEach(
                                user -> userObjects.removeIf(object -> object.equals(user)));
                        }
                    }
                }
            }
