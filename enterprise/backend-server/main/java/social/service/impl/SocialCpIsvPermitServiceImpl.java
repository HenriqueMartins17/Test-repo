...
public class SocialCpIsvPermitServiceImpl implements ISocialCpIsvPermitService {
    ...
    private static final String WEBHOOK_CONTENT_ORDER_CREATED =
        "系统已自动为企业（**企业 Corp ID：%s**）创建新购订单（**订单编号：%s**），请及时前往企业微信服务商后台完成付款。";

    private static final String WEBHOOK_CONTENT_ORDER_MANUAL =
        "系统自动为企业（**企业 Corp ID：%s**）创建新购订单失败，请及时人工操作下单，空间站 ID：%s，应购月数：%s。";

    private static final String WEBHOOK_CONTENT_RENEWAL_CREATED =
        "系统已自动为企业（**企业 Corp ID：%s**）创建续期订单（**订单编号：%s**），请及时前往企业微信服务商后台完成付款。";

    private static final String WEBHOOK_CONTENT_RENEWAL_MANUAL =
        "系统自动为企业（**企业 Corp ID：%s**）创建续期订单失败，请及时人工操作下单，空间站 ID：%s。";

    private static final String WEBHOOK_CONTENT_REFUND =
        "企业（**企业 Corp ID：%s**）已退款取消购买付费订阅版本，请及时前往企业微信服务商后台对该企业的接口许可订单退款。";
    ...
    public SocialWecomPermitOrderEntity createNewOrder(String spaceId, Integer durationMonths) {
        ...
        List<String> allActiveCodes =
            socialWecomPermitOrderAccountService.getActiveCodes(suiteId, authCorpId, null);
        ...
        try {
            createNewOrder =
                wxCpIsvPermitService.createNewOrder(authCorpId, newAccountCount, durationMonths,
                    buyerUserId);
        ...
        try {
            WxCpIsvPermitGetOrder getOrder =
                wxCpIsvPermitService.getOrder(createNewOrder.getOrderId());
        ...
        SocialWecomPermitOrderEntity orderEntity =
            buildOrderEntity(null, getOrderDetail, suiteId, authCorpId, buyerUserId);
    ...
    public void activateOrder(String orderId) {
        ...
        if (orderEntity.getOrderStatus() == 0) {
            ...
            try {
                ...
                WxCpIsvPermitServiceImpl wxCpIsvPermitService =
                    wxCpIsvService.getWxCpIsvPermitService();
        ...
        List<String> allActivatedCodes =
            socialWecomPermitOrderAccountService.getActiveCodes(suiteId, authCorpId,
        ...
        List<String> allTransferredCodes =
            socialWecomPermitOrderAccountService.getActiveCodes(suiteId, authCorpId,
    ...
    @Override
    public SocialWecomPermitOrderEntity renewalCpUser(String spaceId, List<String> cpUserIds,
        ...
        WxCpIsvPermitCreateRenewOrderRequest renewOrderRequest =
            new WxCpIsvPermitCreateRenewOrderRequest();
        ...
        for (List<String> userIds : CollUtil.split(cpUserIds, 1000)) {
            List<WxCpIsvPermitCreateRenewOrderRequest.AccountList> accountList = userIds.stream()
                .map(userId -> {
                    WxCpIsvPermitCreateRenewOrderRequest.AccountList account =
                        new WxCpIsvPermitCreateRenewOrderRequest.AccountList();
        ...
        try {
            submitRenewOrder =
                wxCpIsvPermitService.submitRenewOrder(renewOrderRequest.getJobId(), durationMonths,
                    buyerUserId);
        } catch (WxErrorException ex) {
        ...
        try {
            WxCpIsvPermitGetOrder getOrder =
                wxCpIsvPermitService.getOrder(submitRenewOrder.getOrderId());
            getOrderDetail = getOrder.getOrder();
        } catch (WxErrorException ex) {
            log.error("Exception occurred while getting order.", ex);
            throw new BusinessException(SocialException.WECOM_ISV_PERMIT_API_ERROR);
    ...
    public int calcNewAccountCount(String suiteId, String authCorpId, String spaceId) {
        List<String> noActivatedCpUserIds =
            socialWecomPermitOrderAccountService.getNeedActivateCpUserIds(suiteId, authCorpId,
                spaceId);
        List<String> availableActiveCodes =
            socialWecomPermitOrderAccountService.getActiveCodes(suiteId, authCorpId,
                Arrays.asList(SocialCpIsvPermitActivateStatus.NO_ACTIVATED.getValue(),
                    SocialCpIsvPermitActivateStatus.TRANSFERRED.getValue()));
        ...
        int newAccountCount =
            CollUtil.size(noActivatedCpUserIds) - CollUtil.size(availableActiveCodes);
        if (newAccountCount < 0) {
            newAccountCount = 0;
        }
    ...
    public SocialWecomPermitOrderEntity ensureOrder(String orderId) {
        ...
        try {
            WxCpIsvServiceImpl wxCpIsvService =
                (WxCpIsvServiceImpl) weComTemplate.isvService(orderEntity.getSuiteId());
            WxCpIsvPermitServiceImpl wxCpIsvPermitService =
                wxCpIsvService.getWxCpIsvPermitService();
            WxCpIsvPermitGetOrder getOrder = wxCpIsvPermitService.getOrder(orderId);
            orderEntity = buildOrderEntity(orderEntity, getOrder.getOrder(), null, null, null);
    ...
    public void autoProcessPermitOrder(String suiteId, String authCorpId, String spaceId) {
        ...
        if (DateTimeUtil.between(tenantEntity.getCreatedAt(), DateTimeUtil.localDateTimeNow(8),
            ...
            if (CollUtil.isNotEmpty(orderEntities)) {
                ...
                boolean result = createPermitOrder(suiteId, authCorpId, spaceId,
    ...
    @Override
    public boolean createPermitOrder(String suiteId, String authCorpId, String spaceId,
        ...
        if (newAccountCount > 0) {
            ...
            try {
                SocialWecomPermitOrderEntity permitOrderEntity =
                    createNewOrder(spaceId, durationMonths);
                // Order information is notified to Lark Group
                TaskManager.me().execute(
                    () -> sendNewWebhook(suiteId, authCorpId, null, permitOrderEntity.getOrderId(),
                        null));
            } catch (Exception ex) {
                log.error("Exception occurred while creating new order.", ex);
                // To be notified to Lark group about manually placing an order
                TaskManager.me().execute(
                    () -> sendNewWebhook(suiteId, authCorpId, spaceId, null, durationMonths));
            }
        }
        // Judge whether to renew according to the expiration time of the activation code
        Map<Integer, List<String>> needRenewCpUserIds =
            getNeedRenewCpUserIds(suiteId, authCorpId, spaceId, expireTime);
        if (CollUtil.isNotEmpty(needRenewCpUserIds)) {
            ...
            try {
                for (Entry<Integer, List<String>> entry : needRenewCpUserIds.entrySet()) {
                    SocialWecomPermitOrderEntity permitOrderEntity =
                        renewalCpUser(spaceId, entry.getValue(), entry.getKey());
                    // Notify Lark group of order information
                    TaskManager.me().execute(() -> sendRenewWebhook(suiteId, authCorpId, null,
                        permitOrderEntity.getOrderId()));
                }
            } catch (Exception ex) {
                log.error("Exception occurred while creating renewal order.", ex);
                // To be notified to Lark group about manually placing an order
                TaskManager.me()
                    .execute(() -> sendRenewWebhook(suiteId, authCorpId, spaceId, null));
            }
        }
        return result;
    }
    ...
    @Override
    public boolean sendNewWebhook(String suiteId, String authCorpId, String spaceId, String orderId,
                                  Integer durationMonths) {
        IsvApp isvApp = weComProperties.getIsvAppList().stream()
            .filter(isv -> isv.getSuiteId().equals(suiteId))
            .findFirst()
            .orElse(null);
        return sendWebhook(isvApp.getPermitNotifyWebhookUrl(),
            isvApp.getPermitNotifyWebhookSecret(), authCorpId, spaceId,
            orderId, durationMonths, CharSequenceUtil.isNotBlank(orderId), true, false);
    }

    @Override
    public boolean sendRenewWebhook(String suiteId, String authCorpId, String spaceId,
                                    String orderId) {
        IsvApp isvApp = weComProperties.getIsvAppList().stream()
            .filter(isv -> isv.getSuiteId().equals(suiteId))
            .findFirst()
    ...
    public boolean sendRefundWebhook(String suiteId, String authCorpId) {
        IsvApp isvApp = weComProperties.getIsvAppList().stream()
            ...
            .orElse(null);
        return sendWebhook(isvApp.getPermitNotifyWebhookUrl(),
            isvApp.getPermitNotifyWebhookSecret(), authCorpId, null,
    ...
    private void saveAllActiveCodes(String suiteId, String authCorpId, String orderId) {
        ...
        socialWecomPermitOrderAccountService.batchSaveActiveCode(suiteId, authCorpId, orderId,
            allAccountList);
    }
    ...
    private void ensureActiveCodes(String suiteId, String authCorpId, List<String> allActiveCodes) {
        ...
        for (List<String> activeCodes : CollUtil.split(allActiveCodes, 1000)) {
            ...
            try {
                batchGetActiveInfo =
                    wxCpIsvPermitService.batchGetActiveInfo(authCorpId, activeCodes);
            } catch (WxErrorException ex) {
        ...
        CollUtil.split(allActiveCodes, 500).forEach(activeCodes -> {
            List<SocialWecomPermitOrderAccountEntity> accountEntities =
                socialWecomPermitOrderAccountService
            ...
            accountEntities.forEach(accountEntity -> {
                ...
                if (Objects.isNull(activeInfo)) {
                    ...
                    int actualActivateStatus =
                        SocialCpIsvPermitActivateStatus.fromWecomStatus(activeInfo.getStatus())
                            .getValue();
                    String actualCpUserId = activeInfo.getUserId();
                    LocalDateTime actualCreateTime =
                        DateTimeUtil.localDateTimeFromSeconds(activeInfo.getCreateTime(), 8);
                    LocalDateTime actualActiveTime =
                        DateTimeUtil.localDateTimeFromSeconds(activeInfo.getActiveTime(), 8);
                    LocalDateTime actualExpireTime =
                        DateTimeUtil.localDateTimeFromSeconds(activeInfo.getExpireTime(), 8);
     ...
     */
    private void activateAccount(String suiteId, String authCorpId, String spaceId,
                                 List<String> activeCodes) {
        // 1 Get the member information to be activated
        List<String> needActivateCpUserIds =
            socialWecomPermitOrderAccountService.getNeedActivateCpUserIds(suiteId, authCorpId,
                spaceId);
        ...
        WxCpIsvServiceImpl wxCpIsvService = (WxCpIsvServiceImpl) weComTemplate.isvService(suiteId);
        WxCpIsvPermitServiceImpl wxCpIsvPermitService = wxCpIsvService.getWxCpIsvPermitService();
        WxCpIsvPermitBatchActiveAccountRequest batchActiveAccountRequest =
            new WxCpIsvPermitBatchActiveAccountRequest();
     ...
     */
    private SocialWecomPermitOrderEntity buildOrderEntity(
        SocialWecomPermitOrderEntity originalEntity, WxCpIsvPermitGetOrder.Order getOrderDetail,
        ...
        if (Objects.isNull(originalEntity)) {
            return SocialWecomPermitOrderEntity.builder()
                ...
                .createTime(
                    DateTimeUtil.localDateTimeFromSeconds(getOrderDetail.getCreateTime(), 8))
            ...
            originalEntity.setPayTime(
                DateTimeUtil.localDateTimeFromSeconds(getOrderDetail.getPayTime(), 8));
    ...
    private int calcDurationMonths(LocalDateTime currentTime, LocalDateTime expiredTime) {
        int durationDays =
            (int) DateTimeUtil.between(currentTime, expiredTime, ChronoField.EPOCH_DAY);
     ...
     */
    private Map<Integer, List<String>> getNeedRenewCpUserIds(String suiteId, String authCorpId,
                                                             String spaceId,
                                                             LocalDateTime expireTime) {
        List<SocialWecomPermitOrderAccountEntity> needRenewAccounts =
            socialWecomPermitOrderAccountService
        ...
        return needRenewAccounts.stream()
            ...
            }, Collectors.mapping(SocialWecomPermitOrderAccountEntity::getCpUserId,
                Collectors.toList())));
    }
    ...
    public boolean sendWebhook(String webhookUrl, String webhookSecret,
                               String authCorpId, String spaceId, String orderId,
                               Integer durationMonths,
        ...
        if (CharSequenceUtil.isNotBlank(webhookSecret)) {
            ...
            byte[] signature =
                DigestUtil.hmac(HmacAlgorithm.HmacSHA256, signData.getBytes(StandardCharsets.UTF_8))
                    .digest(new byte[] {});
        ...
        if (isOrderCreated) {
            if (isNew) {
                elementContentText.addProperty("content",
                    String.format(WEBHOOK_CONTENT_ORDER_CREATED, authCorpId, orderId));
            } else {
                elementContentText.addProperty("content",
                    String.format(WEBHOOK_CONTENT_RENEWAL_CREATED, authCorpId, orderId));
            }
        } else if (isRefund) {
            elementContentText.addProperty("content",
                String.format(WEBHOOK_CONTENT_REFUND, authCorpId));
        } else {
            if (isNew) {
                elementContentText.addProperty("content",
                    String.format(WEBHOOK_CONTENT_ORDER_MANUAL, authCorpId, spaceId,
                        durationMonths));
            } else {
                elementContentText.addProperty("content",
                    String.format(WEBHOOK_CONTENT_RENEWAL_MANUAL, authCorpId, spaceId));
            }
        }
        ...
        if (isOrderCreated || isRefund) {
            ...
            elementActionRedirect.addProperty("url",
                "https://open.work.weixin.qq.com/wwopen/developer#/sass/license/service/order/list");
        ...
        JsonObject responseBody = Optional.ofNullable(response)
            ...
            .orElse(null);
        JsonElement statusCode =
            Objects.isNull(responseBody) ? null : responseBody.get("StatusCode");
