...
import static com.apitable.enterprise.social.redis.RedisKey.getWecomIsvContactDepartListKey;
import static com.apitable.enterprise.social.redis.RedisKey.getWecomIsvContactTagGetKey;
import static com.apitable.enterprise.social.redis.RedisKey.getWecomIsvContactUserGetKey;
import static com.apitable.enterprise.social.redis.RedisKey.getWecomIsvContactUserSimpleListKey;
import static com.apitable.enterprise.social.redis.RedisKey.getWecomIsvMemberNewListKey;
...
public class SocialCpIsvServiceImpl implements ISocialCpIsvService {
    ...
    private static final String WX_JSAPI_TICKET_SIGNATURE_SOURCE =
    ...
    public void refreshAccessToken(String suiteId, String authCorpId, String permanentCode,
        ...
        if (isRefresh || forceRefresh) {
            ...
            try {
                if (lock.tryLock(1000L, TimeUnit.MILLISECONDS)) {
                    try {
                        WxAccessToken wxAccessToken =
                            wxCpTpService.getCorpToken(authCorpId, permanentCode);
                        wxCpTpConfigStorage.updateAccessToken(authCorpId,
                            wxAccessToken.getAccessToken(), wxAccessToken.getExpiresIn() / 2);

                        log.info(String.format(
                            "Enterprise【%s】access_token has been updated, the updated value is：%s",
                ...
                log.error(String.format("Tenant [%s] get access_ token failed solve it quickly",
                    authCorpId), ex);
    ...
    @Override
    public WeComIsvJsSdkConfigVo getJsSdkConfig(SocialTenantEntity socialTenantEntity, String url)
        ...
        String source =
            StrUtil.format(WX_JSAPI_TICKET_SIGNATURE_SOURCE, jsApiTicket, random, timestamp, url);
        // 3 Autograph
    ...
    @Override
    public WeComIsvJsSdkAgentConfigVo getJsSdkAgentConfig(SocialTenantEntity socialTenantEntity,
    ...
    public void createAuthFromPermanentInfo(String suiteId, String authCorpId,
        ...
        String spaceId = socialTenantBindService.getTenantBindSpaceId(tenantEntity.getTenantId(),
            tenantEntity.getAppId());
        ...
        if (CharSequenceUtil.isBlank(spaceId)) {
            ...
            SpaceEntity spaceEntity = createWeComIsvSpaceWithoutUser(
                authCorpInfo.getCorpName() + SPACE_NAME_DEFAULT_SUFFIX);
            isNewSpace = true;
            spaceId = spaceEntity.getSpaceId();
            // 2.2 Bind the newly created space
            socialTenantBindService.addTenantBind(tenantEntity.getAppId(),
                tenantEntity.getTenantId(), spaceId);
        ...
        iSocialService.setContactSyncing(spaceId, authCorpInfo.getCorpId());
        ...
        syncViewableUsers(suiteId, authCorpInfo.getCorpId(), spaceId,
            allowUsers, allowParties, allowTags, tenantEntity, agent.getAgentId());
        ...
        if (isNewSpace) {
            MemberEntity memberEntity =
                memberService.getBySpaceIdAndOpenId(spaceId, authUserInfo.getUserId());
        ...
        EditionInfo.Agent editionInfo =
            SocialFactory.filterWecomEditionAgent(permanentCodeInfo.getEditionInfo());
        ...
        if (CharSequenceUtil.isNotBlank(templateNodeId)) {
            nodeService.copyNodeToSpace(-1L, spaceId, rootNodeId, templateNodeId,
    ...
    public SpaceEntity createWeComIsvSpaceWithoutUser(String spaceName) {
        ...
        SpaceEntity space =
            SocialFactory.createSocialBindBindSpaceInfo(spaceId, spaceName, null, null, null);
        boolean isSaved = iSpaceService.save(space);
    ...
    public void createNewSpace(String suiteId, String authCorpId) {
        SocialTenantBindEntity tenantBindEntity =
            socialTenantBindService.getByTenantIdAndAppId(authCorpId, suiteId);
        ...
        SpaceEntity deletedSpace =
            spaceService.getBySpaceIdIgnoreDeleted(tenantBindEntity.getSpaceId());
        ...
        SpaceEntity newSpace = createWeComIsvSpaceWithoutUser(deletedSpace.getName());
        ...
        if (Objects.nonNull(deletedAdminMember)) {
            syncSingleUser(authCorpId, deletedAdminMember.getOpenId(), suiteId,
                newSpace.getSpaceId(), true);
        }
        ...
        String templateNodeId = templateService.getDefaultTemplateNodeId();
        if (CharSequenceUtil.isNotBlank(templateNodeId)) {
            nodeService.copyNodeToSpace(-1L, newSpace.getSpaceId(), rootNodeId, templateNodeId,
                NodeCopyOptions.create());
        }
    }
    ...
    public boolean judgeViewable(String corpId, String cpUserId, String suiteId,
                                 List<String> allowUsers, List<Integer> allowParties,
        ...
        String userGetKey = getWecomIsvContactUserGetKey(corpId);
        ...
        if (CollUtil.isNotEmpty(allowTags)) {
            WxCpIsvTagServiceImpl wxCpTpTagService =
                (WxCpIsvTagServiceImpl) weComTemplate.isvTagService(suiteId);
            // 3.1 Traverse labels in all visible ranges
            for (Integer allowTag : allowTags) {
                // Use cache to prevent frequent calls to the interface during mass operation of the address book
                String tagGetKey = getWecomIsvContactTagGetKey(corpId);
                String tagGetHashKey = allowTag.toString();
                WxCpTpTagGetResult tagGetResult =
                    Optional.ofNullable(hashOperations.get(tagGetKey, tagGetHashKey))
                ...
                if (judgeDepartViewable(suiteId, corpId, tagGetResult.getPartylist(),
                    departmentIds)) {
                    return true;
                }
            }
    ...
    public void syncViewableUsers(String suiteId, String authCorpId, String spaceId,
                                  List<String> allowUsers, List<Integer> allowParties,
                                  List<Integer> allowTags, SocialTenantEntity socialTenantEntity,
                                  Integer agentId)
        throws WxErrorException {
        ...
        while (CollUtil.isNotEmpty(members)) {
            for (MemberEntity member : members) {
                ...
                boolean viewable =
                    judgeViewable(authCorpId, cpUserId, suiteId, allowUsers, allowParties,
                        allowTags);
                if (!viewable) {
        ...
        fetchAndBindAllViewableUsers(suiteId, authCorpId, spaceId, allowUsers, allowParties,
            allowTags, socialTenantEntity, agentId);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void syncSingleUser(String corpId, String cpUserId, String suiteId, String spaceId,
                               boolean isAdmin) {
        ...
        if (Objects.isNull(memberEntity)) {
            try {
                ...
                boolean isNotOverLimit =
                    iSpaceService.checkSeatOverLimitAndSendNotify(ListUtil.toList(mainAdminUserId),
                        spaceId, 1l, false, false);
                if (!isNotOverLimit) {
            ...
            UserEntity userEntity = Optional.ofNullable(
                    socialCpUserBindService.getUserIdByTenantIdAndAppIdAndCpUserId(corpId, suiteId,
                        cpUserId))
            ...
            memberEntity = MemberEntity.builder()
                ...
                .nameModified(false)
                .isSocialNameModified(Objects.isNull(userEntity) ||
                    !isSocialNameModified(userEntity.getIsSocialNameModified()) ?
            ...
            teamMemberRelService.addMemberTeams(Collections.singletonList(memberEntity.getId()),
                Collections.singletonList(rootTeamId));
            // 1.4 Add to the new member cache list
            String memberNewListKey = getWecomIsvMemberNewListKey(corpId);
    ...
    public void clearCache(String authCorpId) {
        List<String> keys = Arrays.asList(
            getWecomIsvContactUserGetKey(authCorpId),
            getWecomIsvContactDepartListKey(authCorpId),
            getWecomIsvContactTagGetKey(authCorpId),
            getWecomIsvContactUserSimpleListKey(authCorpId),
            getWecomIsvMemberNewListKey(authCorpId)
        );
        stringRedisTemplate.delete(keys);
    }

    @Override
    public void sendWelcomeMessage(SocialTenantEntity socialTenantEntity, String spaceId,
                                   WxCpMessage wxCpMessage,
                                   List<String> toUsers, List<Integer> toParties,
                                   List<Integer> toTags)
        throws WxErrorException {
        ...
        // If necessary, refresh access_token
        refreshAccessToken(socialTenantEntity.getAppId(), socialTenantEntity.getTenantId(),
        ...
        if (KefuMsgType.NEWS.equals(wxCpMessage.getMsgType())) {
            ...
            for (MpnewsArticle mpnewsArticle : wxCpMessage.getMpnewsArticles()) {
                mpnewsArticle.setContentSourceUrl(
                    fillingSendWeComMsgUrl(mpnewsArticle.getContentSourceUrl(), variable));
            }
        } else {
        ...
        TaskManager.me().execute(() -> {
            WxCpIsvServiceImpl wxCpTpService =
                (WxCpIsvServiceImpl) weComTemplate.isvService(socialTenantEntity.getAppId());
            try {
                WxCpMessageSendResult sendResult =
                    wxCpTpService.sendMessage(socialTenantEntity.getTenantId(), wxCpMessage);
                log.info("WeCom third-party service provider sends message, parameter{}, Result:{}",
                    wxCpMessage.toJson(), sendResult.toString());
    ...
    public void sendWelcomeMessage(SocialTenantEntity socialTenantEntity, String spaceId,
                                   WxCpMessage wxCpMessage)
        throws WxErrorException {
        String memberNewListKey = getWecomIsvMemberNewListKey(socialTenantEntity.getTenantId());
        ListOperations<String, String> listOperations = stringRedisTemplate.opsForList();
        int offset = 0;
    ...
    @Override
    public void sendMessageToUser(SocialTenantEntity socialTenantEntity, String spaceId,
                                  WxCpMessage wxCpMessage,
    ...
    @Override
    public void sendTemplateMessageToUser(SocialTenantEntity socialTenantEntity, String spaceId,
                                          WxCpMessage wxCpMessage, List<String> toUsers)
        throws WxErrorException {
        if (ObjectUtil.hasNull(socialTenantEntity, spaceId)) {
            throw new IllegalArgumentException("Parameter cannot be empty");
        }
        ...
        WxCpMessageSendResult sendResult =
            wxCpTpService.sendMessage(socialTenantEntity.getTenantId(), wxCpMessage);
        log.info("We Com third-party service provider sends message, parameter {}, result:{}",
            wxCpMessage.toJson(), sendResult.toString());
    }

    @Override
    public QueryResult search(String suiteId, String authCorpId, Integer agentId, String keyword,
                              Integer type) throws WxErrorException {
        ...
        while (hasMore) {
            WxCpTpContactSearchResp searchResp =
                wxCpTpContactService.contactSearch(wxCpTpContactSearch);
            ...
            wxCpTpContactSearch.setOffset(
                wxCpTpContactSearch.getOffset() + wxCpTpContactSearch.getLimit());
        }
    ...
    @Override
    public void handleTenantPaidSubscribe(String suiteId, String authCorpId, String spaceId,
        ...
        if (SubscriptionPhase.TRIAL.equals(
            WeComPlanConfigManager.getSubscriptionPhase(paidEvent.getEditionId()))) {
            socialWecomPermitDelayService.addAuthCorp(suiteId, authCorpId,
        ...
        TaskManager.me().execute(() -> {
            ...
            if (SubscriptionPhase.TRIAL.equals(
                ...
                goodChTitle = Objects.requireNonNull(
                        WeComPlanConfigManager.getPriceByWeComEditionIdAndMonth(
                            paidEvent.getEditionId(),
            ...
            SocialNotificationManagement.me().sendSocialSubscribeNotify(spaceId, toUserId,
                ClockUtil.secondToLocalDateTime(paidEvent.getEndTime(), ZoneOffset.ofHours(8))
                    .toLocalDate(),
    ...
    public EditionInfo.Agent getCorpEditionInfo(String authCorpId, String suiteId) {
        String permanentCode =
            iSocialTenantService.getPermanentCodeByAppIdAndTenantId(suiteId, authCorpId);
    ...
    @Override
    public WeComOrderPaidEvent fetchPaidEvent(String suiteId, String orderId)
        throws WxErrorException {
    ...
    private void fetchAndBindAllViewableUsers(String suiteId, String authCorpId, String spaceId,
                                              ...
                                              Integer agentId) throws WxErrorException {
        ...
        List<WxCpUser> partyCpUsers =
            fetchViewableCpUserIdsFromParties(suiteId, authCorpId, allowParties);
        ...
        if (CollUtil.isNotEmpty(allowTags)) {
            ...
            for (Integer allowTag : allowTags) {
                WxCpTpTagGetResult tagGetResult =
                    wxCpTpTagService.get(allowTag.toString(), authCorpId);
                ...
                List<WxCpUser> tagPartyCpUsers =
                    fetchViewableCpUserIdsFromParties(suiteId, authCorpId, tagParties);
        ...
        try {
            boolean isNotOverLimit = iSpaceService.checkSeatOverLimitAndSendNotify(
                Collections.singletonList(mainAdminUserId), spaceId, allCpUserIds.size(), true,
                true);
            if (!isNotOverLimit) {
                // If the seat limit is exceeded, the user will not be synchronized， and throw an exception
                String openId = iSpaceService.getSpaceOwnerOpenId(spaceId);
                if (Objects.nonNull(openId)) {
                    ...
                    WxCpMessage wxCpMessage = WeComIsvCardFactory.createSeatOverLimitMsg(agentId);
                    sendMessageToUser(socialTenantEntity, spaceId, wxCpMessage,
        ...
        for (List<String> cpUserIds : splitCpUserIds) {
            ...
            List<MemberEntity> memberEntities =
                memberService.getBySpaceIdAndOpenIds(spaceId, cpUserIds);
        ...
        for (List<String> addCpUserIds : splitAddCpUserIds) {
            Map<String, UserEntity> cpUserMap =
                socialCpTenantUserService.getUserByCpUserIds(authCorpId, suiteId, addCpUserIds);
            List<MemberEntity> addMemberEntities = addCpUserIds.stream()
                .map(addCpUserId -> {
                    ...
                    return MemberEntity.builder()
                        ...
                        .memberName(
                            Objects.isNull(userEntity) ? addCpUserId : userEntity.getNickName())
                        .mobile(Objects.isNull(userEntity) ? null : userEntity.getMobilePhone())
                        .email(Objects.isNull(userEntity) ? null : userEntity.getEmail())
                        .openId(addCpUserId)
                        .status(UserSpaceStatus.INACTIVE.getStatus())
            ...
            List<Long> addMemberIds = addMemberEntities.stream()
                ...
                .collect(Collectors.toList());
            teamMemberRelService.addMemberTeams(addMemberIds,
                Collections.singletonList(rootTeamId));
        }
    }
     ...
     * @return Member List
     */
    private List<WxCpUser> fetchViewableCpUserIdsFromParties(String suiteId, String authCorpId,
                                                             List<Integer> partyIds) {
        if (CollUtil.isEmpty(partyIds)) {
            return Collections.emptyList();
        }

        WxCpIsvUserServiceImpl wxCpTpUserService =
            (WxCpIsvUserServiceImpl) weComTemplate.isvService(suiteId)
                .getWxCpTpUserService();
        // Use cache to prevent frequent calls to the interface during mass operation of the address book
        HashOperations<String, String, String> hashOperations = stringRedisTemplate.opsForHash();
        String userSimpleListKey = getWecomIsvContactUserSimpleListKey(authCorpId);

        return partyIds.stream()
            .flatMap(partyId -> {
                String userSimpleListHashKey = partyId.toString();
                List<WxCpUser> cpUsers = Optional.ofNullable(
                        hashOperations.get(userSimpleListKey, userSimpleListHashKey))
                    .map(string -> JSONUtil.toList(string, WxCpUser.class))
                    .orElse(null);
                if (CollUtil.isEmpty(cpUsers)) {
                    try {
                        cpUsers =
                            wxCpTpUserService.listSimpleByDepartment(partyId.longValue(), true,
                                WeComUserStatus.ACTIVE.getCode(), authCorpId);
                        hashOperations.put(userSimpleListKey, userSimpleListHashKey,
                            JSONUtil.toJsonStr(cpUsers));
                    } catch (WxErrorException ex) {
     ...
     */
    private void bindSpaceAdmin(AuthCorpInfo authCorpInfo, AuthUserInfo authUserInfo,
                                WxCpTpPermanentCodeInfo.Agent agent,
     ...
     */
    private boolean judgeDepartViewable(String suiteId, String authCorpId,
                                        List<Integer> allowDepartIds, List<Integer> cpDepartIds)
        throws WxErrorException {
        HashOperations<String, String, String> hashOperations = stringRedisTemplate.opsForHash();
        ...
        // Use cache to prevent frequent calls to the interface during mass operation of the address book
        String departListKey = getWecomIsvContactDepartListKey(authCorpId);
        for (Integer allowDepartId : allowDepartIds) {
            ...
            List<WxCpTpDepart> wxCpTpDeparts =
                Optional.ofNullable(hashOperations.get(departListKey, departListHashKey))
            ...
            if (Objects.isNull(wxCpTpDeparts)) {
                ...
                hashOperations.put(departListKey, departListHashKey,
                    JSONUtil.toJsonStr(wxCpTpDeparts));
            }
