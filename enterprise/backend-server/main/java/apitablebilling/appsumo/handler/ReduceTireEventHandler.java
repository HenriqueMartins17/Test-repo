...
/**
 * appsumo event handler: reduce_tire(downgrade).
 */
@AppsumoEventHandler(action = AppsumoAction.REDUCE_TIER)
@ConditionalOnProperty(value = "appsumo.enabled", havingValue = "true")
public class ReduceTireEventHandler implements IAppsumoEventHandler {
    ...
    public EventVO handle(Long eventLogId) {
        ...
        // allow user to reduce tire when not activated
        if (bundles.isEmpty()) {
            return EventVO.builder().message(DEFAULT_SUCCESS_MESSAGE).build();
        }
