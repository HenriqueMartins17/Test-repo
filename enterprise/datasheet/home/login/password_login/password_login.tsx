...
import { IPasswordData, IPasswordLoginConfig, IPasswordModes, PasswordLoginModes } from './password_login_modes';
...
const initMode = (supportAccountType: IdentifyingCodeModes | undefined): IdentifyingCodeModes => {
  if (supportAccountType) {
    const defaultMode = supportAccountType.split(',')[0];
    if (defaultMode !== 'qrcode') {
      return defaultMode as IdentifyingCodeModes;
    }
  }
...
export const PasswordLogin: FC<IPasswordLoginProps> = ({ submitRequest, config }) => {
  ...
  const handleModeChange = (mode: IPasswordModes) => {
    setErrMsg(defaultErrMsg);
    setMode(mode);
  };
