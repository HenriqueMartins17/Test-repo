import { WidgetProps, RegistryWidgetsType } from '@rjsf5/utils';
...
const SelectFormWidget = function (props: WidgetProps) {
  ...
  return (
    props.value && treeNodesMap[props.value] ? (
      ...
      <SelectForm rootId={rootId} secondConfirmType={SecondConfirmType.AIForm} onChange={(event) => props.onChange(event)}>
        {(onClick) => (
          <div className={styles.selectDst} onClick={onClick}>
            <AddFilled />
            <Typography style={{ marginLeft: 8 }} color={color.textCommonTertiary}
              variant="body3">{t(Strings.ai_select_form)}</Typography>
          </div>
        )}
      </SelectForm>
