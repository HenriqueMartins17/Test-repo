...
######
###### build airagent version product
######
function build_airagent_web {
  export DEFUALT_SEMVER_EDITION="vika"

  _build "https://s4.aitable.ai" "${QS_APITABLE_BUCKET_NAME}"
}
...
function _build {
  ...
  export DOCKERFILE=packaging/Dockerfile.airagent-web
  ...
  build_docker_unableack dotversion airagent-web
  ...
  docker cp $(docker create --name next_tmp "${TEMP_CP_DOCKER_IMAGE}"):/app/packages/air-agent/web_build/static /tmp/app/"$SEMVER_FULL" &&
    docker cp next_tmp:/app/packages/air-agent/public /tmp/app/"$SEMVER_FULL" &&
    docker rm next_tmp
...
function _upload_source_map {
  local sentry_project="airagent-web"
  local sentry_url="$1"
