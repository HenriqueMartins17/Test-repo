...
// ----- databus-wasm transform -----

json.transform = function (op, otherOp, type) {
    return JSON.parse(databusInstance.json0_transform(JSON.stringify(op), JSON.stringify(otherOp), JSON.stringify(type)));
};

// ----- databus-wasm apply -----

json.apply = function (snapshot, op) {
    return JSON.parse(databusInstance.json0_apply(JSON.stringify(snapshot), JSON.stringify(op)));
};

// ----- databus-wasm compose -----

json.compose = function (op1, op2) {
    return JSON.parse(databusInstance.json0_compose(JSON.stringify(op1), JSON.stringify(op2)));
};

// ----- databus-wasm invert -----

json.invert = function (op) {
    return JSON.parse(databusInstance.json0_invert(JSON.stringify(op)));
};
...
//             if (isArray(elem) && typeof key !== 'number') throw new Error('List index must be a number');

//             if (isObject(elem) && typeof key !== 'string') throw new Error('Object key must be a string');
