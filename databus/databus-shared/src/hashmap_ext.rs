...
pub struct HashMapExt<K: Hash + Eq, V>(pub std::collections::HashMap<K, V, BuildSeaHasher>);
...
impl<K, V> Default for HashMapExt<K, V>
where
  K: Hash + Eq,
{
  fn default() -> Self {
...
impl<K, V> DerefMut for HashMapExt<K, V>
where
  K: Hash + Eq,
{
...
impl<K, V> FromIterator<(K, V)> for HashMapExt<K, V>
where
  K: Eq + Hash,
{
...
impl<K, V> PartialEq for HashMapExt<K, V>
where
  K: Eq + Hash,
  V: PartialEq,
{
...
impl<K, V> Eq for HashMapExt<K, V>
where
  K: Eq + Hash,
  V: Eq,
{
...
impl<K, V> IntoIterator for HashMapExt<K, V>
where
  K: Hash + Eq,
{
  type Item = (K, V);
...
impl<'a, K, V> IntoIterator for &'a HashMapExt<K, V>
where
  K: Hash + Eq,
{
  type Item = (&'a K, &'a V);
...
impl<'a, K, V> IntoIterator for &'a mut HashMapExt<K, V>
where
  K: Hash + Eq,
{
...
impl<K, V> Extend<(K, V)> for HashMapExt<K, V>
where
  K: Eq + Hash,
{
  #[inline]
