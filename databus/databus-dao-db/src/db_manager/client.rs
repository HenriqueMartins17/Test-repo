...
impl DbClient {
  ...
  pub async fn execute<S>(&mut self, sql: S, params: Params) -> anyhow::Result<()>
  where
    S: Into<String>,
  {
    let sql = sql.into();
    match &mut self.inner {
      DbClientInner::Mysql(conn) => Ok({
  ...
  /// Execute the insert sql
  pub async fn execute_with_transaction<S>(&mut self, query: Vec<(S, Params)>) -> anyhow::Result<()>
  where
    S: Into<String>,
  {
    match &mut self.inner {
      DbClientInner::Mysql(conn) => Ok({
        ...
        for x in query.into_iter() {
          let (sql, params) = x;
          transaction.exec_drop(sql.into(), params).await?;
        }
