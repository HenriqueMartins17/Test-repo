...
use databus_core::fields::property::{
  CreatedByFieldPropertySO, FormulaFieldPropertySO, LinkFieldPropertySO, LookupFieldPropertySO, MemberFieldPropertySO,
};
use databus_core::prelude::{FieldKindSO, FieldSO, RecordSO};
use databus_shared::prelude::{HashMapExt, HashSet};
use databus_shared::prelude::Json;
use databus_shared::prelude::OptionBoolExt;
...
pub(super) struct DependencyAnalyzer<'a> {
  ...
  main_dst_record_map: Arc<Mutex<HashMap<String, RecordSO>>>,
...
struct DependencyAnalysisWork {
  dst_id: String,
  field_map: Arc<HashMap<String, FieldSO>>,
  record_map: Arc<Mutex<HashMap<String, RecordSO>>>,
...
impl<'a> DependencyAnalyzer<'a> {
  ...
  async fn analyze_field_dependencies(
    ...
    for field_id in field_ids {
      ...
      match field.kind {
        FieldKindSO::Link => {
          let property = property.to_link_field_property();
        ...
        FieldKindSO::LookUp => {
          let property: LookupFieldPropertySO = property.to_lookup_field_property();
          ...
          if lookuped_field.kind != FieldKindSO::Link {
            continue;
          }
          ...
          let LinkFieldPropertySO {
            foreign_datasheet_id, ..
          } = lookuped_field_property.to_link_field_property();
          let mut foreign_field_ids = hashset![];
          foreign_field_ids.insert(property.look_up_target_field_id);
          ...
          if property.open_filter.is_truthy() {
            if let Some(filter_into) = property.filter_info {
              for condition in filter_into.conditions {
                foreign_field_ids.insert(condition.field_id.clone());
              }
            }
          }
        ...
        FieldKindSO::Member => {
          let property: MemberFieldPropertySO = property.to_member_field_property();
          self.member_field_unit_ids.extend(property.unit_ids);
        }
        FieldKindSO::CreatedBy | FieldKindSO::LastModifiedBy => {
          let property: CreatedByFieldPropertySO = property.to_created_by_field_property();
  ...
  async fn load_foreign_datasheet_packs(
    ...
    let mut sort_map = HashMapExt::default();
    for (link_field_id, foreign_dst_id) in &*linked_dst_ids{
      sort_map.insert(link_field_id.clone(), foreign_dst_id.clone());
    }
    for (link_field_id, foreign_dst_id) in &*sort_map {
  ...
  async fn process_linked_dst_primary_fields(
    ...
    for (field_id, foreign_dst_id) in linked_dst_ids {
      ...
      let foreign_primary_field_id = foreign_first_view.columns[0].field_id.clone();
      ...
      if foreign_primary_field.kind == FieldKindSO::Formula {
  ...
  async fn process_formula_field(
    ...
    field_map: Arc<HashMap<String, FieldSO>>,
    field: &FieldSO,
    record_map: Option<Arc<Mutex<HashMap<String, RecordSO>>>>,
    new_works: &mut Vec<DependencyAnalysisWork>,
  ) -> anyhow::Result<()> {
    let Some(property) = field.property.as_ref() else {
      return Ok(());
    ...
    let FormulaFieldPropertySO {
      expression,
      datasheet_id,
      ..
    } = property.to_formula_field_property();
...
mod tests {
  ...
  use databus_core::prelude::{BaseDatasheetPackSO, DatasheetSnapshotSO, RecordSO};
  ...
  use super::super::reference_manager::mock::{MockReferenceManagerImpl, MockRefKey};
  ...
  async fn single_datasheet_collect_users() {
    ...
    let meta = InternalDatasheetMeta {
      field_map: Arc::new(hashmap_standard! {
    ...
    let record_map = Arc::new(Mutex::new(hashmap_standard! {}));
    ...
    assert_eq!(ref_man.lock().await.refs, hashmap_standard! {});
  }
  ...
  mod self_linking {
    ...
    fn mock_dst2_dst_pack(record_ids: Option<HashSet<&str>>) -> BaseDatasheetPackSO {
      BaseDatasheetPackSO {
        snapshot: DatasheetSnapshotSO {
          meta: DatasheetMetaSO {
            field_map: hashmap_standard! {
              "fld2w1".into() => new_field("fld2w1", FieldKindSO::Formula, json!({
                "datasheetId": "dst2",
                "expression": "+{fld2w40000000}",
              })),
              "fld2w2".into() => new_field("fld2w2", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst1",
                "brotherFieldId": "fld1w2",
              })),
              "fld2w3".into() => new_field("fld2w3", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst1",
                "brotherFieldId": "fld1w3",
              })),
              "fld2w40000000".into() => new_field("fld2w40000000", FieldKindSO::Link, json!({
    ...
    fn mock_dst1_field_map() -> Arc<HashMap<String, FieldSO>> {
      Arc::new(hashmap_standard! {
        "fld1w1".into() => new_field("fld1w1", FieldKindSO::Text, json!({})),
        "fld1w2".into() => new_field(
          "fld1w2",
          FieldKindSO::Link,
    ...
    async fn two_link_fields_in_one_datasheet_link_same_datasheet_transitive_records() {
      ...
      assert_eq!(
        ...
        hashmap_standard! {
          MockRefKey::new("dst1", "fld1w2") => hashmap_standard! {
            "dst2".into() => vec![hashset!["fld2w1".into()]],
          },
          MockRefKey::new("dst1", "fld1w3") => hashmap_standard! {
            "dst2".into() => vec![hashset!["fld2w1".into()]],
          },
          MockRefKey::new("dst2", "fld2w1") => hashmap_standard! {
            "dst2".into() => vec![hashset!["fld2w40000000".into()]],
          },
          MockRefKey::new("dst2", "fld2w40000000") => hashmap_standard! {
            "dst2".into() => vec![hashset!["fld2w1".into()]],
          },
        }
      );
    ...
    fn mock_dst3_dst_pack(record_ids: Option<HashSet<&str>>) -> BaseDatasheetPackSO {
      BaseDatasheetPackSO {
        snapshot: DatasheetSnapshotSO {
          meta: DatasheetMetaSO {
            field_map: hashmap_standard! {
              "fld3w1".into() => new_field("fld3w1", FieldKindSO::Formula, json!({
                "datasheetId": "dst3",
                "expression": "{fld3w20000000} + {fld3w40000000}",
              })),
              "fld3w20000000".into() => new_field("fld3w20000000", FieldKindSO::LookUp, json!({
                "datasheetId": "dst3",
                "relatedLinkFieldId": "fld3w3",
                "lookUpTargetFieldId": "fld3w5",
              })),
              "fld3w3".into() => new_field("fld3w3", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst3",
              })),
              "fld3w40000000".into() => new_field("fld3w40000000", FieldKindSO::Member, json!({
                "isMulti": true,
                "shouldSendMsg": false,
                "unitIds": ["u1", "u2", "u3"]
              })),
              "fld3w5".into() => new_field("fld3w5", FieldKindSO::Formula, json!({
                "datasheetId": "dst3",
                "expression": "{fld3w60000000}",
              })),
              "fld3w60000000".into() => new_field("fld3w60000000", FieldKindSO::CreatedBy, json!({
                "uuids": ["11", "12", "13", "15"],
                "datasheetId": "dst3"
              })),
              "fld3w7".into() => new_field("fld3w7", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst1",
                "brotherFieldId": "fld1w2",
              })),
            },
    ...
    fn mock_lookup_self_dst1_field_map() -> Arc<HashMap<String, FieldSO>> {
      Arc::new(hashmap_standard! {
        "fld1w1".into() => new_field("fld1w1", FieldKindSO::Text, json!({})),
        "fld1w2".into() => new_field(
          "fld1w2",
    ...
    async fn lookup_relates_same_datasheet_self_linking_field_transitive_records() {
      ...
      assert_eq!(
        ...
        hashmap_standard! {
          ...
          MockRefKey::new("dst3", "fld3w1") => hashmap_standard! {
            "dst3".into() => vec![hashset!["fld3w20000000".into(), "fld3w40000000".into()]],
          },
          MockRefKey::new("dst3", "fld3w20000000") => hashmap_standard! {
            "dst3".into() => vec![hashset!["fld3w5".into()]],
          },
          MockRefKey::new("dst3", "fld3w3") => hashmap_standard! {
            "dst3".into() => vec![hashset!["fld3w1".into()]],
          },
          MockRefKey::new("dst3", "fld3w5") => hashmap_standard! {
    ...
    fn mock_dst4_dst_pack(record_ids: Option<HashSet<&str>>) -> BaseDatasheetPackSO {
      BaseDatasheetPackSO {
        snapshot: DatasheetSnapshotSO {
          meta: DatasheetMetaSO {
            field_map: hashmap_standard! {
              "fld4w1".into() => new_field("fld4w1", FieldKindSO::Text, json!({})),
              "fld4w2".into() => new_field("fld4w2", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst1",
                "brotherFieldId": "fld1w2",
              })),
              "fld4w3".into() => new_field("fld4w3", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst4",
              })),
              "fld4w4".into() => new_field("fld4w4", FieldKindSO::Member, json!({
                "isMulti": true,
                "shouldSendMsg": false,
                "unitIds": ["u1", "u2"]
              })),
    ...
    fn mock_lookup_foreign_dst1_field_map() -> Arc<HashMap<String, FieldSO>> {
      Arc::new(hashmap_standard! {
        "fld1w1".into() => new_field("fld1w1", FieldKindSO::Text, json!({})),
        "fld1w2".into() => new_field("fld1w2", FieldKindSO::Link, json!({
          "foreignDatasheetId": "dst4",
          "brotherFieldId": "fld4w2",
        })),
        "fld1w3".into() => new_field("fld1w3", FieldKindSO::LookUp, json!({
          "datasheetId": "dst1",
    ...
    async fn lookup_relates_foreign_datasheet_self_linking_field_transitive_records() {
      ...
      assert_eq!(
        ...
        hashmap_standard! {
          ...
          MockRefKey::new("dst4", "fld4w3") => hashmap_standard! {
            "dst4".into() => vec![hashset!["fld4w1".into()]],
          },
        }
      );
  ...
  mod lookup {
    ...
    async fn lookup_related_field_invalid() {
      ...
      let field_map = Arc::new(hashmap_standard! {
        "fld1w1".into() => new_field("fld1w1", FieldKindSO::Text, json!({})),
        "fld1w2".into() => new_field("fld1w2", FieldKindSO::LookUp, json!({
          "datasheetId": "dst1",
        ...
        "fld1w5".into() => new_field("fld1w5", FieldKindSO::LookUp, json!({
          "datasheetId": "dst1",
      ...
      assert_eq!(
        ref_man.lock().await.refs,
        hashmap_standard! {
          MockRefKey::new("dst1", "fld1w5") => hashmap_standard! {
            "dst4".into() => vec![hashset!["fld4w3".into(), "fld4w4".into()]],
          },
        }
      );
    ...
    fn mock_dst2_dst_pack(record_ids: Option<HashSet<&str>>) -> BaseDatasheetPackSO {
      BaseDatasheetPackSO {
        snapshot: DatasheetSnapshotSO {
          meta: DatasheetMetaSO {
            field_map: hashmap_standard! {
              "fld2w1".into() => new_field("fld2w1", FieldKindSO::Text, json!({
              })),
              "fld2w2".into() => new_field("fld2w2", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst1",
                "brotherFieldId": "fld1w2",
              })),
              "fld2w3".into() => new_field("fld2w3", FieldKindSO::Formula, json!({
                "datasheetId": "dst2",
                "expression": "+{fld2w40000000}",
              })),
              "fld2w40000000".into() => new_field("fld2w40000000", FieldKindSO::Link, json!({
              ...
              "fld2w7".into() => new_field("fld2w7", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst3",
    ...
    fn mock_dst3_dst_pack(record_ids: Option<HashSet<&str>>) -> BaseDatasheetPackSO {
      BaseDatasheetPackSO {
        snapshot: DatasheetSnapshotSO {
          meta: DatasheetMetaSO {
            field_map: hashmap_standard! {
              "fld3w1".into() => new_field("fld3w1", FieldKindSO::Text, json!({})),
              "fld3w2".into() => new_field("fld3w2", FieldKindSO::LookUp, json!({
                "datasheetId": "dst3",
                "relatedLinkFieldId": "fld3w3",
              ...
              "fld3w4".into() => new_field("fld3w4", FieldKindSO::Link, json!({
                "foreignDatasheetId": "dst1",
    ...
    async fn reprocess_dirty_fields() {
      ...
      assert_eq!(
        ...
        hashmap_standard! {
          ...
          MockRefKey::new("dst1", "fld1w4") => hashmap_standard! {
            "dst3".into() => vec![hashset!["fld3w1".into()]],
          },
          MockRefKey::new("dst2", "fld2w3") => hashmap_standard! {
            "dst2".into() => vec![hashset!["fld2w40000000".into()]],
          },
          MockRefKey::new("dst1", "fld1w5") => hashmap_standard! {
          ...
          // MockRefKey::new("dst2", "fld2w7") => hashmap_standard! {
          //   "dst3".into() => vec![hashset!["fld3w1".into()]],
          // },
          MockRefKey::new("dst2", "fld2w40000000") => hashmap_standard! {
            "dst2".into() => vec![hashset!["fld2w1".into()], hashset!["fld2w1".into()]],
          },
          MockRefKey::new("dst3", "fld3w2") => hashmap_standard! {
            "dst2".into() => vec![hashset!["fld2w40000000".into()]],
          },
          MockRefKey::new("dst3", "fld3w3") => hashmap_standard! {
            "dst2".into() => vec![hashset!["fld2w1".into()]],
  ...
  mod extend_records {
    ...
    fn mock_dst1_dst_pack(record_ids: Option<HashSet<&str>>) -> BaseDatasheetPackSO {
      BaseDatasheetPackSO {
        snapshot: DatasheetSnapshotSO {
          meta: DatasheetMetaSO {
            field_map: hashmap_standard! {
        ...
        snapshot: DatasheetSnapshotSO {
          meta: DatasheetMetaSO {
            field_map: hashmap_standard! {
              ...
              "fld2w3".into() => new_field("fld2w3", FieldKindSO::LookUp, json!({
                "datasheetId": "dst2",
